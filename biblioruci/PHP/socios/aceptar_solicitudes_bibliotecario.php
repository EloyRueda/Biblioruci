...
<?php
    ...
    //si consulta_bibliotecario está vacío, el usuario no es bibliotecario
    if(!$consulta_bibliotecario){
        session_abort();
...
<body>
    ...
    <?php
        if(isset($_POST['id_socio'])) {
            if("SELECT id FROM socios WHERE aceptado = '0'" != null & mysqli_real_escape_string($conectar, $_POST['id_socio']) != null){
                ...
                $consulta_aceptar = "UPDATE socios SET aceptado = '1' WHERE id = $id_socio AND categoria = 'cliente'";
                //realizar la consulta anterior
                $insertar = mysqli_query($conectar, $consulta_aceptar);
            }
            else{
                echo "El ID introducido no pertenece a ningún socio no aceptado";
            }
        }
        else{
    ...
    <table border="1" align="center" id="solicitudes">
        ...
        <?php
            ...
            //código para mostrar las solicitudes de los clientes
