...
<?php
    ...
    $seleccionar1 = "SELECT * FROM socios WHERE categoria = 'cliente' AND correo_electronico = '$_SESSION[correo_electronico]' AND aceptado = '1'";
    $seleccionsql1 = mysqli_query($conectar, $seleccionar1);
    $contador1 = mysqli_num_rows($seleccionsql1);

    $seleccionar2 = "SELECT * FROM socios WHERE categoria = 'bibliotecario' AND correo_electronico = '$_SESSION[correo_electronico]' AND aceptado = '1'";
    $seleccionsql2 = mysqli_query($conectar, $seleccionar2);
    $contador2 = mysqli_num_rows($seleccionsql2);

    $seleccionar3 = "SELECT * FROM socios WHERE categoria = 'administrador' AND correo_electronico = '$_SESSION[correo_electronico]' AND aceptado = '1'";
...
<head>
    ...
    <title>Sesión cliente | Biblioteca</title>
</head>
...
<body>
    ...
    <form action="#" method="POST" id="preguntar" style="font-size: 1.5em; text-align: center; background-color: #b3a369; border: 5px solid #6b5e30; padding-bottom: 30px; margin-top: 30px; margin-bottom: 30px;">
        <h2>Preguntar</h2>
        <textarea type="text" class="entrada_formulario" name="pregunta" id="pregunta" placeholder="Introduzca su pregunta" rows="3" cols="90" required></textarea>
        <br>
    ...
    <?php
        if($contador1 == 1 || $contador2 == 1 || $contador3 == 1){
            //recoger el valor del campo de texto en una variable
            $pregunta = mysqli_real_escape_string($conectar, $_POST['pregunta']);

            if($pregunta){
                //definir consulta para añadir la pregunta a la base de datos
                $consulta_pregunta = "INSERT INTO preguntas (correo_pregunta, pregunta) VALUES ('$correo_electronico', '$pregunta')";
                //ejecutar la consulta anterior
                $insertar_pregunta = mysqli_query($conectar, $consulta_pregunta);

                if($insertar_pregunta){
                    echo "<h2>Su pregunta ha sido enviada correctamente</h2>";
                }
                else{
                ...
                echo "Introduzca su pregunta";
            }

        } //fin if($contador1 == 1 || $contador2 == 1 || $contador3 == 1)
        else{
            header("Location: ../../index.html"); //expulsar al usuario si no tiene una categoría válida
        }
