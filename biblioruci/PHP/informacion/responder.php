...
<head>
    ...
    <style>
        #formulario_pregunta_a_responder{
            font-size: 1.5em;
            text-align: center;
...
<body>
    ...
    <h1 class="titulo">Responder preguntas</h1>

    <form action="#" id="formulario_pregunta_a_responder" method="POST">
        <input type="number" class="entrada_formulario" name="id_pregunta" id='id_pregunta' placeholder="ID de la pregunta" size="20%" required> <br>
        <input type="submit" id="responder_pregunta" value="Responder pregunta"></input>
    </form>

    <table border="1" align="center" id="preguntas">
        <tr style='font-size:1.5em;'>
            <th><u>ID pregunta</u></th>
            <th><u>Correo electrónico cliente</u></th>
            <th><u>Pregunta</u></th>
        </tr>
        <?php
            //definir consulta para mostrar las preguntas no respondidas
            $consulta_preguntas = "SELECT * FROM preguntas WHERE respuesta IS NULL";
            ...
            while($fila_pregunta = mysqli_fetch_array($resultados_preguntas)){
                echo "<tr style='font-size:1.2em;'>"
                    ."<td>".$fila_pregunta["id_pregunta"]."</td>"
                    ."<td>".$fila_pregunta["correo_pregunta"]."</td>"
    ...
    <?php
        //código para enviar a una página para responder a la pregunta cuyo ID se ha introducido
        if(isset($_POST["id_pregunta"])){
            
            $id_pregunta_a_responder = mysqli_real_escape_string($conectar, $_POST['id_pregunta']);
            if($id_pregunta_a_responder != ""){
            
                if("SELECT * FROM preguntas WHERE id = $id_pregunta_a_responder" != ""){
                    $_SESSION["id_pregunta_a_responder"] = $id_pregunta_a_responder;
                    $_SESSION["pregunta_a_responder"] = "SELECT pregunta FROM preguntas WHERE id_pregunta = $id_pregunta_a_responder";
                    header("location: ./responder_2.php");
                }
                else{
                    echo "<h3>El ID de la pregunta introducida no existe</h3>";
                }
            } //fin if($id_pregunta_a_responder != "")
            else{
                echo "<h3>La pregunta introducida no ha sido cuestionada o ya se ha respondido</h3>";
            }
        }//fin if(isset($_POST["id_pregunta"]))
        else{
            echo "<h3>Introduzca el ID de la pregunta a responder</h3>";
        }
    ?>
</body>
</html>